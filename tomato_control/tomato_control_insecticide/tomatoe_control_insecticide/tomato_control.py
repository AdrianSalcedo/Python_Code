...
class OptimalControlProblem(object):
    ...
    def g(self, x_k, u_k):
        ...
        rhs_s_p = beta * (l_p + i_p) - a * s_p * i_v
        rhs_l_p = a * s_p * i_v - b * l_p - beta * l_p
        rhs_i_p = b * l_p - beta * i_p
        rhs_s_v = - Lambda * i_p * s_v - (gamma + u_2) * s_v + (1 - theta) * mu
        rhs_i_v = Lambda * i_p * s_v - (gamma + u_2) * i_v + theta * mu
    ...
    def lambda_function(self, x_k, u_k, lambda_k):
        ...
        rhs_l_3 = - A_1 + beta * (lambda_3 - lambda_1) + Lambda * s_v * (lambda_4 - lambda_5)

        rhs_l_4 = Lambda * i_p * (lambda_4 - lambda_5) + (gamma+ u_2) * lambda_4

        rhs_l_5 = -A_3 + a * s_p * (lambda_1 - lambda_2) + (gamma + u_2) * lambda_5
    ...
    def optimality_condition(self, x_k, u_k, lambda_k, n_max):
        ...
        aux_2 =  (lambda_4 * s_v + lambda_5* i_v) / (2 * c_1)
